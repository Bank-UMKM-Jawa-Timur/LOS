...
<div class="flex justify-between items-center flex-wrap">
    ...
    <div>
      <button class="fullscreen">
        <iconify-icon
          ...
          class="text-2xl text-gray-500 mt-[8px]"
        ></iconify-icon>
      </button>
    </div>
    @php
      $user_id = auth()->user()->id;
      $notifRepo = new \App\Repository\NotificationsRepository;
      $notification = $notifRepo->getWithLimit($user_id);
    @endphp
  ...
  <div class="notification-list hidden border lg:w-96 w-80 bg-white absolute top-14 lg:right-32 right-0">
      ...
      @endforelse
      <a href="{{route('dagulir.notification.index')}}">
        <div class="footer-notif text-center p-3 hover:bg-gray-300">
            Lihat semua
        </div>
      </a>
  </div>
