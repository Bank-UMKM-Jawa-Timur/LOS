            ...
            <form action="{{ route('dagulir.master.kecamatan.store') }}" method="POST">
                ...
                <div class="form-group-1 mb-4">
                    <div class="input-box">
                        ...
                        <input type="text" name="kecamatan" class="form-input @error('kecamatan') is-invalid @enderror" placeholder="Nama Kecamatan" value="{{old('kecamatan')}}">
                    </div>
                    <div class="input-box">
                        ...
                        <select name="id_kabupaten" class="form-select" id="id_kabupaten">
                            ...
                            @foreach ($allKab as $kab)
                                <option value="{{ $kab->id }}"
                                    {{ old('id_kabupaten') == $kab->id ? ' selected' : '' }}>{{ $kab->kabupaten }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_kabupaten')
