...
<section class="">
    ...
    <div class="p-3">
        <div class="body-pages">
            <form action="{{ route('dagulir.pengajuan.update', $dataUmum?->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="id_dagulir_temp" id="id_dagulir_temp" value="{{ $dataUmum?->id }}">
                <div class="mt-3 container mx-auto">
                    ...
                    @foreach ($dataAspek as $key => $value)
                        @php
                            ...
                            ->where('id_parent', $value->id)
                            ->get();
                            $nib = edit_dagulir($dataUmum->id, 77)?->opsi_text;
                            $sku = edit_dagulir($dataUmum->id, 78)?->opsi_text;
                            $npwp = edit_dagulir($dataUmum->id, 79)?->opsi_text;
                        @endphp
