<div class="table-responsive">
  <table class="table table-hover table-custom">
      ...
      <tbody>
          ...
          @endphp
          @foreach ($dataProdukKredit as $item)
              <tr class="border-bottom-primary">
                  <td class="text-center text-muted">{{ $no }}</td>
                  <td>{{ $item->name }}</td>
                  <td>
                      <div class="form-inline btn-action">
                          <a href="{{ route('produk-kredit.edit', $item->id) }}" class="mr-2">
                          ...
                          <button type="button" class="btn btn-rgb-danger btn-sm" data-toggle="modal" data-target="#confirmDeleteModal{{$item->id}}">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <form action="{{ route('produk-kredit.destroy', $item->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
  ...
  <div class="pull-right">
        {{ $dataProdukKredit->appends(Request::all())->links('vendor.pagination.custom') }}
  </div>
</div>
