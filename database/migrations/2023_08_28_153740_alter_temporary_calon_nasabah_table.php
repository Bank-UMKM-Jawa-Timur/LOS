...
class AlterTemporaryCalonNasabahTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $pkpj_query = "UPDATE temporary_calon_nasabah SET produk_kredit_id = IF(skema_kredit = 'PKPJ', $pkpj_id, produk_kredit_id)";
        DB::statement($pkpj_query);
        $kkb_query = "UPDATE temporary_calon_nasabah SET produk_kredit_id = IF(skema_kredit = 'KKB', $kkb_id, produk_kredit_id)";
        DB::statement($kkb_query);
        $umroh_query = "UPDATE temporary_calon_nasabah SET produk_kredit_id = IF(skema_kredit = 'Talangan Umroh', $umroh_id, produk_kredit_id)";
        DB::statement($umroh_query);
        $prokesra_query = "UPDATE temporary_calon_nasabah SET produk_kredit_id = IF(skema_kredit = 'Prokesra', $prokesra_id, produk_kredit_id)";
        DB::statement($prokesra_query);
        $kusuma_query = "UPDATE temporary_calon_nasabah SET produk_kredit_id = IF(skema_kredit = 'Kusuma', $kusuma_id, produk_kredit_id)";
    ...
    {
        DB::statement("UPDATE temporary_calon_nasabah SET produk_kredit_ID = NULL");
        
        Schema::table('temporary_calon_nasabah', function(Blueprint $table) {
            $table->dropForeign('temporary_calon_nasabah_produk_kredit_id_foreign');
            $table->dropForeign('temporary_calon_nasabah_skema_kredit_id_foreign');
            $table->dropForeign('temporary_calon_nasabah_skema_limit_id_foreign');
        ...
        DB::table('mst_produk_kredit')->truncate();
    }
}
